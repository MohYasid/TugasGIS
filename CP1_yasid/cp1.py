...
POSTGIS_TABLE = dict(
	...
	table='(select ST_Buffer(ST_Centroid(geom),1) as geom, nama from tabel2) as tabel2'
)
ds = mapnik.PostGIS(**POSTGIS_TABLE)
layer = mapnik.Layer('indonesia')
layer.datasource = ds 
layer.styles.append('Yasid2')
m.layers.append(layer)
